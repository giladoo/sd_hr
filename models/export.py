...
class SdHrExport(models.Model):
        ...
        if len(records) > 1 or attach_docs:
            ...
            with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
                for rec in records:
                    ...
                    if attach_docs:
                        documents = hr_documents.search(
                            [('employee_id', '=', rec.employee_id.id), ('resume_document', '=', True)])
                        for document in documents:
                            for index, att in enumerate(document.attachments):
                                ...
                                zip_file.writestr(
                                    f"{zip_folder}/{document.document_type.name}_({index}).{att.name.split('.')[-1]}",
            ...
            doc_content = self.regenerate_template(record, variable_no,
                                                   output_type)  # Your function to create `.docx` content
    ...
    def _replace_and_format(self, paragraph, placeholder, value, bold=False, font_name=B_NAZANIN, italic=False,
